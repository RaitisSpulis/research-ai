...
function getCategoryContent(category, topic, seed) {
  const templates = {
    startup: {
      ...
      monetization: "Freemium entry with Pro exports, templates, and team libraries at $29–99/mo.",
      findings: [
        ["Demand signal", `Users researching ${topic} prefer actionable reports over raw search results.`],
      ...
      risks: [
        ...
        ["Trust barrier", "Source visibility and confidence scoring are essential."],
      ...
      actions: [
        ...
        ["Build trust layer", "Add source cards, confidence badges, and assumption labels."],
        ["Launch Pro tier", "Gate PDF/DOCX exports and unlimited reports behind Pro."],
        ["Expand templates", "Ship 12 vertical templates for founders, analysts, and students."]
      ],
      sources: [
        ["Industry reports", "Gartner, McKinsey, and vertical market research databases."],
        ["Competitor sites", "Pricing pages, product docs, and customer review platforms."],
        ["Public filings", "SEC filings, press releases, and analyst coverage where applicable."]
      ]
    },
    realestate: {
    ...
    investment: {
      executive: `Our research on <strong>${escapeHtml(topic)}</strong> outlines a balanced investment view. Catalysts include product momentum and market expansion, while risks center on valuation, competition, and macro sensitivity. This report is a structured framework — not financial advice.`,
      ...
      findings: [
        ...
        ["Valuation context", "Current multiples vs 5-year historical range inform entry timing."],
      ...
      sources: [
        ["SEC EDGAR", "10-K, 10-Q, and proxy filings."],
        ["Earnings calls", "Management commentary and Q&A transcripts."],
        ["Analyst research", "Sell-side and independent equity research reports."]
      ]
    },
    learning: {
  ...
  return {
    ...
    disadvantages: [
      ["Data limitations", "Demo mode uses estimated figures — verify with primary sources."],
...
function buildReportHTML(data) {
  ...
  return `
    ...
    <section class="report-section">
      <div class="section-title"><h2>Executive Summary</h2><span>High confidence</span></div>
      <p>${data.executive}</p>
    ...
    <section class="report-section" id="findings">
      <div class="section-title"><h2>Key Findings</h2><span>Verified structure</span></div>
    ...
    <section class="report-section" id="analysis">
      ...
      <p>
        ...
        signals derived from your research question — verify critical assumptions with primary data.
      </p>
    ...
    <section class="report-section" id="sources">
      <div class="section-title"><h2>Sources</h2><span>Demo mode</span></div>
      <div class="insight-grid">${sourcesHtml}</div>
      <p class="report-disclaimer">In production, sources would link to verified references. This beta generates structured estimates locally.</p>
    </section>`;
}
...
function renderReport(data) {
  ...
  els.reportMeta.textContent = `Generated ${relativeTime(data.createdAt)} · 9 sections · ${data.confidence}% confidence`;
  els.metricConfidence.textContent = `${data.confidence}%`;
  els.metricDepth.textContent = data.confidence >= 88 ? "Expert" : "Professional";
...
function updatePreview(prompt) {
  if (!prompt.trim()) {
    ...
    els.previewScore.setAttribute("aria-label", "Confidence not yet calculated");
  ...
  els.previewScore.textContent = `${data.confidence}%`;
  els.previewScore.setAttribute("aria-label", `Estimated confidence ${data.confidence} percent`);
...
function renderRecentReports() {
  ...
  els.continueText.textContent = `Continue "${latest.title}" with deeper pricing, competitors and source analysis.`;
}
...
function startResearch() {
  ...
  if (getUsage() >= FREE_LIMIT) {
    showToast("Free limit reached (5/month). Pro coming soon.", "warn");
    return;
  }

  showInputError("");
  currentPrompt = prompt;
  els.loadingPrompt.textContent = `ResearchAI is analyzing: "${prompt}"`;
  ...
  if (name === "templates") {
    ...
    els.panelBody.innerHTML = `
      <div class="settings-group">
        <h3>Workspace</h3>
        <p>Reports are stored locally in your browser. No data is sent to a server.</p>
      ...
      <div class="settings-group">
        <h3>About</h3>
        <p>ResearchAI Public Beta — demo mode. All reports are generated locally using your input.</p>
  ...
  if (!reports.length) {
    els.panelBody.innerHTML = `
      <div class="empty-state">
        <div class="empty-icon" aria-hidden="true">${favoritesOnly ? "★" : "◷"}</div>
  ...
  els.panelBody.innerHTML = `<div class="history-list">${reports.map(r => `
    <button type="button" class="history-item" data-id="${r.id}">
      ...
      <div>
        ...
        <p>${relativeTime(r.createdAt)} · ${r.confidence}% confidence${r.favorite ? " · ★" : ""}</p>
      </div>
    </button>`).join("")}</div>`;
...
function handleContinueFromCard() {
  ...
  setPrompt(`${continuePrompt} with deeper pricing, competitors and source analysis`);
  startResearch();
}
...
function handleContinueFromReport() {
  ...
  showView("dashboard");
  setTimeout(() => {
    setPrompt(`${currentReport.prompt} with deeper pricing, competitors and source analysis`);
    els.researchInput?.focus();
  }, 300);
}
...
function bindEvents() {
  ...
  document.querySelectorAll("[data-coming-soon]").forEach(btn => {
    btn.addEventListener("click", e => {
      e.stopPropagation();
      showToast(`${btn.dataset.comingSoon} — Coming Soon`, "info");
    });
  });
